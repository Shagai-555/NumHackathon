...
function Home() {
    ...
    useEffect(() => {
        const sortedList = [...feedData].sort((a, b) => new Date(b.dateTime) - new Date(a.dateTime));
        setList(sortedList);
    }, [feedData]);

    const handleIncrement = (id) => {
        setList(feedData => feedData.map(item => {
            if (item.id === id) {
                            ...
                            <div className="flex flex-col w-[650px] my-1 p-2 hover:rounded-2xl hover:bg-gray-800">
                                <div className='flex flex-row justify-between'>
                                    <div className='flex flex-row items-center text-sm'>
                                        ...
                                        {dayDiff === 0 && hoursDifference === 0 && minutesDifference > 0 && (
                                            <span className='text-gray-300 mr-2'>
                                                {`${hoursDifference} цаг ${minutesDifference} минутын өмнө`}
                                            </span>
                                        )}
                                ...
                                {item.img.url && (
                                    <img
                                        src={item.img.url}
                                        alt="issuer"
                                        className="h-[400px] w-[400px] object-cover mx-auto my-auto opacity-70 rounded-xl"
                                        style={{ display: 'block' }} />
                                )}
                                <div className='flex flex-row mt-3 items-center justify-between'>
                                    <div className="flex flex-row">
                                        <div className='flex flex-row bg-gray-700 rounded-2xl items-center mr-3'>
                                            ...
                                            <span className='my-1 text-sm font-bold text-white'>{item.vote}</span>
                                            <button onClick={() => handleIncrement(item.id)}>
                                                <svg className="p-1 hover:rounded-full hover:text-red-900 hover:bg-gray-600" width="30px" height="30px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M7 16L9 16C11.2091 16 13 14.2091 13 12L13 7" stroke="green" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                        ...
                                        <div className="bg-gray-700 p-1 rounded-2xl flex hover:bg-gray-600 items-center">
                                            ...
                                            <path fill-rule="evenodd" clip-rule="evenodd" d="M5.5 12C5.49988 14.613 6.95512 17.0085 9.2741 18.2127C11.5931 19.4169 14.3897 19.2292 16.527 17.726L19.5 18V12C19.5 8.13401 16.366 5 12.5 5C8.63401 5 5.5 8.13401 5.5 12Z" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                    ...
                                    <span className="text-sm text-gray-300 my-2">{item.where}</span>
                                </div>
                            </div>
                        </>
                    );
